...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(
    sql`INSERT INTO \`__new_pages_rels\`("id", "order", "parent_id", "path", "tags_id", "blogs_id", "forms_id") SELECT "id", "order", "parent_id", "path", "tags_id", "blogs_id", "forms_id" FROM \`pages_rels\`;`,
  )
  ...
  await db.run(
    sql`CREATE INDEX \`pages_rels_tags_id_idx\` ON \`pages_rels\` (\`tags_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`pages_rels_blogs_id_idx\` ON \`pages_rels\` (\`blogs_id\`);`,
  )
  await db.run(
  ...
  `)
  await db.run(
    sql`INSERT INTO \`__new__pages_v_rels\`("id", "order", "parent_id", "path", "tags_id", "blogs_id", "forms_id") SELECT "id", "order", "parent_id", "path", "tags_id", "blogs_id", "forms_id" FROM \`_pages_v_rels\`;`,
  )
  ...
  await db.run(
    sql`CREATE INDEX \`_pages_v_rels_tags_id_idx\` ON \`_pages_v_rels\` (\`tags_id\`);`,
  )
  await db.run(
    sql`CREATE INDEX \`_pages_v_rels_blogs_id_idx\` ON \`_pages_v_rels\` (\`blogs_id\`);`,
  )
  await db.run(
  ...
  `)
  await db.run(
    sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "pages_id", "blogs_id", "tags_id", "media_id", "users_id", "forms_id", "form_submissions_id", "search_id") SELECT "id", "order", "parent_id", "path", "pages_id", "blogs_id", "tags_id", "media_id", "users_id", "forms_id", "form_submissions_id", "search_id" FROM \`payload_locked_documents_rels\`;`,
  )
